...
import { Blog, BlogList } from '~/models';

const postsDirectory = path.join(process.cwd(), 'src/posts/blog');

export function getAllBlogsData(): BlogList[] {
  ...
  return allPostsData.sort((a, b) => {
    const _aDate = a.date ? new Date(a.date) : new Date();
    const _bDate = b.date ? new Date(b.date) : new Date();
    if (_aDate < _bDate) {
      return 1;
    } else {
      return -1;
